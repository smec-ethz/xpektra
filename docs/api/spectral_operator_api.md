...
The ``SpectralOperator`` class serves as a unified interface for various spectral operations, including gradient, symmetric gradient, divergence, and Laplacian operations. It also provides methods for forward and inverse Fourier transforms, and the quadrature boundary (``auto_vmap``, ``integrate``). Pointwise tensor algebra lives in ``xpektra.linalg``.
